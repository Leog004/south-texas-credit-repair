...
export default function FeaturedCardsA({data}) {

    const icons = ['FaRegCommentDots', 'FaGlobe', 'FaBalanceScale', 'FaWallet'];
                    ...
                    data.map((el, index) => (
                        ...
                        <div className='flex flex-wrap md:flex-nowrap justify-evenly items-center h-full border-r'>
                            ...
                            <div className='flex flex-col'>
                                <h2 className='text-sm text-center md:text-left font-semibold capitalize'>{el.title}</h2>
                                <p className='text-gray-500 mt-2 text-xs text-center md:text-left'>{el.text}</p>
                            </div>
                        </div>
                    ))
                }
